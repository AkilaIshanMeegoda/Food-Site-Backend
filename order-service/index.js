...
app.post("/orders", verifyToken, async (req, res) => {
    ...
    try {
        ...
        const restaurantResponse = await axios.get(`http://restaurant-service:5001/public/restaurants/${restaurantId}`);
        if (!restaurantResponse.data) {
            return res.status(404).json({ error: "Restaurant not found" });
        }

        // Fetch menu items to validate and calculate total
        const menuItemsResponse = await axios.get(`http://restaurant-service:5001/public/restaurants/${restaurantId}/menu-items`);
